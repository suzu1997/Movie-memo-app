...
export const SignUp: VFC<Props> = (props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = ({email, passward}) => {
    signUpWithEmailAndPassword(email, passward);
    ...
    <>
      <div className='flex flex-col justify-center items-center'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='w-11/12 flex flex-col'>
          ...
          <label className='mt-8'>パスワード<span className='text-sm'> ※6文字以上</span></label>
          <input
            className='w-full p-2 border-2 rounded-md border-gray-400 border-solid focus:outline-none focus:border-black'
        ...
        <button className='text-blue-600 underline mt-4' onClick={() => setIsLogin(true)}>ログインする</button>
      </div>
    </>
  );
};
